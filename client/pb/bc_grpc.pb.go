...
const (
	...
	BrowserControl_StreamScreenshots_FullMethodName = "/pb.BrowserControl/StreamScreenshots"
)
...
type BrowserControlClient interface {
	...
	StreamScreenshots(ctx context.Context, in *ScreenshotRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Screenshot], error)
}
...
func (c *browserControlClient) StreamScreenshots(ctx context.Context, in *ScreenshotRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Screenshot], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &BrowserControl_ServiceDesc.Streams[0], BrowserControl_StreamScreenshots_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[ScreenshotRequest, Screenshot]{ClientStream: stream}
...
type BrowserControl_StreamScreenshotsClient = grpc.ServerStreamingClient[Screenshot]
...
type BrowserControlServer interface {
	...
	// Streaming RPC for continuous screenshots
	StreamScreenshots(*ScreenshotRequest, grpc.ServerStreamingServer[Screenshot]) error
...
func (UnimplementedBrowserControlServer) StreamScreenshots(*ScreenshotRequest, grpc.ServerStreamingServer[Screenshot]) error {
	return status.Errorf(codes.Unimplemented, "method StreamScreenshots not implemented")
}
...
func _BrowserControl_StreamScreenshots_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(ScreenshotRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(BrowserControlServer).StreamScreenshots(m, &grpc.GenericServerStream[ScreenshotRequest, Screenshot]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type BrowserControl_StreamScreenshotsServer = grpc.ServerStreamingServer[Screenshot]
	...
	Streams: []grpc.StreamDesc{
		{
			...
			Handler:       _BrowserControl_StreamScreenshots_Handler,
			ServerStreams: true,
		},
	},
	Metadata: "proto/bc.proto",
}
