...
- `patch`: a small fixed rectangle cycles through eight distinct colors, each held for 250 ms (period 2 s), exercising partial image changes. It changes color four times per second, so its measured write rate is workload-limited and does not measure maximum rendering throughput.
...
Page animation uses wall-clock time and `requestAnimationFrame`, so an overloaded browser can skip animation steps. The patch schedule was previously a two-state toggle that changed color at each logical frame (about every 1/30 s) and returned to the same color every other frame, for a period of about 1/15 s; on loaded CI runners the capture loop settled near 15 Hz — roughly one sample per color period. That aliasing is a reproducible mechanism consistent with the smoke step's failure artifact (30 captures, 30 reuses, zero writes in two seconds), but the artifact does not by itself establish each screenshot's phase; confirmation awaits CI runs of the fixed fixture. The regression tests in `cmd/benchmark/patch_fixture_test.go` execute the actual fixture script under controlled timestamps and animation callbacks, observe the colors it assigns to the patch, and require an observed change within a two-second window at 15 Hz sampling, across all phases, and with dropped animation ticks. Changing this fixture changes its recorded hash (`fixture_sha256`), which the comparison command rejects as a mismatch: end-to-end comparisons require matching fixture versions on both sides. Input-to-paint timing and scripted mouse keys/Vimium interaction are separate future measurements.
