...
6. **Measure delivery and responsiveness.** The performance harness counts captured, reused, superseded, and written frames separately and reports latency distributions, sampled process CPU/RSS, and Go allocation/GC statistics. A static page should not need repeated writes. Account for the built-in 24 FPS ceiling and adaptive waits. Establish visible presentation and input-to-paint timing with a validated terminal-specific or external observation method; do not label successful writes as displayed frames.
